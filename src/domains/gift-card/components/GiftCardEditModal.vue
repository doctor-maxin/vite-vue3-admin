...
import { IGiftCard, IGiftCardPayload } from '../gift-card';
...
interface GiftCardEditModalEmits {
...
const process = async () => {
    if (!isInvalidForm.value) {
        ...
        const result = await giftCardsStore.updateGiftCard(props.entity._id, payload)
...
const emits = defineEmits<GiftCardEditModalEmits>();
const props = defineProps<GiftCardEditModalProps>()
...
onMounted(() => {
    if (props.entity) {
        console.log(props.entity.expiredDate)
        form.balance = props.entity.balance
        form.nominal = props.entity.nominal
        form.code = props.entity.code
        form.email = props.entity.email
        if (props.entity.expiredDate) {
            form.expiredDate = toString(props.entity.expiredDate)
        }
